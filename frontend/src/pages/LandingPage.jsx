...
function BrowserChrome({ url }) {
  return (
    <div style={{
      height: 36, background: '#F8FAFC', borderBottom: '1px solid #E2E8F0',
      ...
      <div style={{
        flex: 1, background: '#FFFFFF', border: '1px solid #E2E8F0', borderRadius: 4,
        height: 20, display: 'flex', alignItems: 'center', padding: '0 10px',
        fontSize: 9, color: '#9CA3AF', fontFamily: 'Space Mono, monospace',
      }}>
        {url}
      </div>
    </div>
...
function InteractiveWalkthrough() {
  ...
  const stepsData = [
    ...
    },
    {
      ...
      desc: "Scores are relative. Compare a company's tone against its sector average or its own historical trend.",
      bullets: ["Sector relative sentiment index", "Prepared vs Q&A split scoring", "Guidance drift alerts"]
    }
  ];

  return (
    <div style={{
      maxWidth: '1000px', margin: '0 auto', display: 'grid', gridTemplateColumns: '320px 1fr', gap: '40px', alignItems: 'start'
    }}>
        ...
        {stepsData.map((step, i) => (
          <div
            ...
            style={{
              ...
              border: '1px solid',
              borderColor: activeStep === i ? '#E2E8F0' : 'transparent',
              boxShadow: activeStep === i ? '0 4px 12px rgba(0,0,0,0.05)' : 'none',
              transition: 'all 200ms ease'
            }}
          >
            <div style={{ display: 'flex', gap: '12px', alignItems: 'center', marginBottom: '8px' }}>
               <div style={{ width: '24px', height: '24px', borderRadius: '50%', background: activeStep === i ? '#C8922A' : '#E2E8F0', color: activeStep === i ? '#FFFFFF' : '#9CA3AF', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px', fontWeight: 600, fontFamily: 'Space Mono' }}>{i + 1}</div>
               <div style={{ fontSize: '16px', fontWeight: 600, color: activeStep === i ? '#0C1628' : '#9CA3AF', fontFamily: 'Inter' }}>{step.title}</div>
            </div>
            {activeStep === i && (
              <div style={{ animation: 'fadeIn 0.3s ease-out' }}>
                <p style={{ fontSize: '14px', color: '#6B7280', lineHeight: 1.6, margin: '8px 0 16px 0' }}>{step.desc}</p>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                   {step.bullets.map(b => (
                     <div key={b} style={{ display: 'flex', gap: '8px', alignItems: 'center', fontSize: '12px', color: '#374151' }}>
                        <div style={{ width: '4px', height: '4px', borderRadius: '50%', background: '#C8922A' }} />
                        {b}
                     </div>
                   ))}
                </div>
      ...
      <div style={{ background: '#FFFFFF', border: '1px solid #E2E8F0', borderRadius: '16px', height: '480px', boxShadow: '0 10px 40px rgba(0,0,0,0.08)', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
          <BrowserChrome url="earninglens.vercel.app/company/HDFCBANK/Q4_FY25" />
          <div style={{ flex: 1, padding: '24px', overflow: 'hidden' }}>
             {activeStep === 0 && (
               <div style={{ animation: 'fadeIn 0.4s ease-out' }}>
                  <div style={{ borderLeft: '3px solid #C8922A', background: '#FEF3C7', padding: '20px', borderRadius: '0 8px 8px 0' }}>
                     <div style={{ fontSize: '10px', color: '#C8922A', textTransform: 'uppercase', marginBottom: '8px', fontFamily: 'Space Mono' }}>Analyst Brief</div>
                     <div style={{ fontSize: '14px', lineHeight: 1.8, color: '#374151' }}>Management tone for HDFC Bank in Q4 FY25 was constructive with an overall score of 0.76, up from 0.73 last quarter. Revenue metrics remained strong while margin language softened on elevated credit costs. Guidance for FY26 remains optimistic with a focus on retail loan growth.</div>
                  </div>
               </div>
             )}
             {activeStep === 1 && (
               <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', animation: 'fadeIn 0.4s ease-out' }}>
                  {[
                    ...
                    <div key={i} style={{ padding: '12px', borderLeft: `3px solid ${row.type === 'positive' ? '#059669' : row.type === 'negative' ? '#DC2626' : '#E2E8F0'}`, background: row.type === 'positive' ? '#ECFDF5' : row.type === 'negative' ? '#FEF2F2' : 'transparent', borderRadius: '0 4px 4px 0' }}>
                       <div style={{ fontSize: '7px', textTransform: 'uppercase', color: row.type === 'positive' ? '#059669' : row.type === 'negative' ? '#DC2626' : '#9CA3AF', marginBottom: '4px' }}>{row.type}</div>
                       <div style={{ fontSize: '12px', color: '#374151' }}>{row.text}</div>
                    </div>
                  ))}
               </div>
             )}
               ...
               <div style={{ animation: 'fadeIn 0.4s ease-out' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '32px' }}>
                     <div style={{ padding: '20px', background: '#F8FAFC', borderRadius: '12px', textAlign: 'center' }}>
                        <div style={{ fontSize: '10px', color: '#9CA3AF', textTransform: 'uppercase' }}>Overall Score</div>
                        <div style={{ fontSize: '32px', fontWeight: 700, color: '#059669', margin: '8px 0' }}>0.76</div>
                        <div style={{ fontSize: '11px', color: '#059669' }}>+0.03 vs prior</div>
                     </div>
                     <div style={{ padding: '20px', background: '#F8FAFC', borderRadius: '12px', textAlign: 'center' }}>
                        <div style={{ fontSize: '10px', color: '#9CA3AF', textTransform: 'uppercase' }}>Sector Avg</div>
                        <div style={{ fontSize: '32px', fontWeight: 700, color: '#374151', margin: '8px 0' }}>0.71</div>
                        <div style={{ fontSize: '11px', color: '#6B7280' }}>BANKING Sector</div>
                     </div>
                  </div>
                  <div style={{ height: '140px', background: '#F8FAFC', borderRadius: '12px', padding: '20px' }}>
                      <div style={{ fontSize: '10px', color: '#9CA3AF', textTransform: 'uppercase', marginBottom: '16px' }}>8-Quarter Trajectory</div>
                      <div style={{ height: '80px', display: 'flex', alignItems: 'flex-end', gap: '10px' }}>
                         {[40, 45, 60, 55, 70, 65, 80, 76].map((h, i) => (
                           <div key={i} style={{ flex: 1, height: `${h}%`, background: i === 7 ? '#059669' : '#E2E8F0', borderRadius: '2px' }} />
                         ))}
                      </div>
                  </div>
               </div>
...
export default function LandingPage() {
  ...
  return (
    <div style={{ background: colors.bgPrimary, color: colors.textPrimary, fontFamily: fonts.body }}>
      ...
      <section ref={sec3Ref} style={{ background: '#FFFFFF', padding: '80px', borderBottom: '1px solid #E2E8F0' }}>
